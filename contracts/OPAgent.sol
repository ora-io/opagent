...
contract OPAgent is AIOracleCallbackReceiver, IOPAgent {
  ...
  event OPAgentCallbackEvent(uint256 requestId, bytes output, bytes callbackData);
  ...
  function estimateERC20ModelFee() public view returns (address token, uint256 fee) {
    (, , fee, , , , token, ) = IAIOracle(aiOracle).getModel(
      modelId
    );
  }
  ...
  function aiOracleCallback(
    ...
    emit OPAgentCallbackEvent(requestId, output, callbackData);
